...
load("Benchmarking Results.RData")
```
...
```{r chessboard, echo = FALSE, fig.show = 'asis', fig.cap= "\\label{fig:fig2-chessboard}Chessboard example"}
#Generate data
...
Interactive Basis Functions (IBFs)
============
...
In this section we conduct a benchmarking exercise using a collection of publicly available datasets that are widely used in the literature to test the performance of machine learning techniques. The datasets are listed in Table \ref{tab:Datasets}.

```{r dataset-description, echo = FALSE}
#create a table with the description of the datasets used for benchmarking
dataset_df <- data.frame(Dataset = c("Balance Scale", "Wine", "BUPA",  "PIMA Indian", "Glass",  "Book Club"),
                         `Quantitative Features` = c(4, 13, 6, 8, 9, 9),
                         `Qualitative Features` = c(0, 0, 0, 0, 0, 1),
                         `Number of Cases` = c(nrow(BalanceScale), nrow(wine), nrow(bupa), nrow(PimaIndiansDiabetes), nrow(Glass), nrow(BBBClub)),
                         Source = c("UCI Machine Learning Repository", "Package rattle.data", "Package dprep", "Package mlbench", "Package mlbench", "Package mlbench")
                         )
kable(dataset_df, "latex", booktabs = TRUE,
...
We report results regarding accuracy, size of the resulting trees, and computer time. With respect to performance, Figure \ref{fig:fig8-performance-benchmark-results-train}) shows the accuracy (in percentage) of the various implementations of the methods. It can be seen there that introducing IBFs leads at least to comparable performance, and more often results in performance gains - with the gains in some cases being quite substantial. This is true regardless of whether the method is the conventional or evolutionary tree.

```{r fig8-performance-benchmark-train-results, echo=FALSE, fig.cap="\\label{fig:fig8-performance-benchmark-results-train}Benchmarking results: Classification accuracy", fig.height=8, fig.show='asis'}
ggplot(data = filter(class.results, method != "Random Forest"), aes(x = method, y = train_class, fill = bf)) +
  geom_boxplot() +
  xlab("Method") +
  ylab("Accuracy (%)") +
  facet_wrap(~dataset, nrow = 3, ncol = 3, scales = "free") +
  theme_minimal() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) 
```

The gains in accuracy do not necessarily imply more complex trees. As seen in Figure \ref{fig:fig9-performance-benchmark-size-results}, the _size_ of the tree, measured in terms of the number of terminal nodes, can increase at times (for example, in the case of the BUPA and PIMA datasets). Other times, the use of IBFs can result in substantial gains in accuracy simultaneously with a modest reduction in the size of the tree (as in the case of the Glass dataset).

```{r fig9-performance-benchmark-size-results, echo=FALSE, fig.cap="\\label{fig:fig9-performance-benchmark-size-results}Benchmarking results: Tree size", fig.height=8, fig.show='asis'}
ggplot(data = filter(class.results, method != "Random Forest"), aes(x = method, y = tree_size, fill = bf)) +
  geom_boxplot() +
  xlab("Method") +
...
The last result that we report concerns computer time. This part of the experiment was conducted using the `microbenchmark` package [@Mersmann2018]. This package times the evaluation of expressions with sub-millisecond accuracy. Each method/dataset/partition type (orthogonal, IBF) was evaluated 50 times to produce the results shown in Figure \ref{fig:fig10-time-benchmark-results}. As seen there, the time required to train trees with IBFs is comparable or occasionally somewhat more favorable than with oblique partitions, perhaps as a consequence of finding better partitions in fewer dimensions.

```{r fig10-time-benchmark-results, echo=FALSE, fig.cap="\\label{fig:fig10-time-benchmark-results}Benchmarking results: Computing time", fig.height=8, fig.show='asis'}
ggplot(data = filter(mbm, expr != "randomForest"), aes(x = expr, y = time, fill = bf)) + 
  geom_boxplot() + 
  xlab("Method") +
  ylab("Time (ms)") +
  facet_wrap(~ dataset, nrow = 3, ncol = 3, scales = "free")  + 
  scale_x_discrete(labels=c("Tree" = "Tree", "evtree" = "Evolutionary Tree")) +
  theme_minimal() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) 
```
...
## Classification Example: Ethnic Neighborhoods
...
Next, the same dataset is used to train a tree with IBFs as listed above. The model was examined to determine that pruning was not required. The results of this model are shown in Figure \ref{fig14-tree-basis-newark}. The model also has six terminal nodes which identify two German neighborhoods, three White American neigborhoods, and one Irish neighborhoods. Note that two of the partitions are orthogonal (i.e., $lat < -0.37$ and $lat \ge -0.16$), two partitions are linear but oblique (i.e., $long+lat \ge 0.51$ and $long+lat \ge 0.21$), and one partition is non-linear (i.e., $e^{long}+e^{lat} \ge 2.0$). The ethnic neighborhoods are shown in Figure \ref{fig15-map-basis-newark}. This model has an in-sample error rate of `r round(0.56087 * 0.70414, digits = 2)` and a cross-validation error rate of `r round(0.56087 * 0.70373, digits = 2)`, which is comparable to the orthogonal model. In this case, the analyst must decide whether the neighborhoods identified by the DT with IBFs are a more realistic representation of a spatial process.

```{r fig14-tree-basis-newark, echo=FALSE, fig.cap="\\label{fig14-tree-basis-newark}Decision tree with non-orthogonal/non-linear partitions for spatial classification, Newark ethnic groups", message=FALSE}
...
```{r fig15-map-basis-newark, fig.cap="\\label{fig15-map-basis-newark}Ethnic neighborhoods in Newark, 1880, using non-orthogonal partitions", echo=FALSE}
#predict using mod2
#create a grid for prediction
...
## Regression Example: Spatial Market Segmentation
...
```{r fig16-map-sapporo, cache = TRUE, echo = FALSE, message = FALSE, fig.cap="\\label{fig:fig16-map-sapporo}Land prices in Sapporo (log)"}
#Plot geocoded observations usig ggmap
ggmap(sapporo.bm) + 
...
## Regression Example: Voter Turnout
...
```{r fig23-election-decision-charts, fig.cap="\\label{fig23-election-decision-charts}Decision charts for voter turnout", warning=FALSE, echo=FALSE}
